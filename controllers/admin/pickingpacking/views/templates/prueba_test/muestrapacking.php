...
<div class="container container_productos">  
    ...
    <?php }?>
        ...
        <!-- 06/11/2020 Añadimos clase = a id_product_id_atributo_id_order para buscar la imagen del producto desde back.js y mostrar la imagen del producto cuando se escanea el ean correctamente-->
        <!-- <div class="col-sm-5 col-xs-4">  -->
        <div class="col-sm-4 col-xs-3">           
            <img src="<?= $imagen ?>"  width="218" height="289"  class="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>"/>    
        </div>
        ...
        <div class="col-sm-5 col-xs-6">
            ...
            <span style="font-size:16px;"><?= $producto['customizable_data'] ?></span><br/>
            <?php } ?>  
        ...
        <?php if ($producto['cantidad'] != 1) {  ?>        
            <div class="alert alert-danger">          
                <p class="h2"><span id="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>"><?= $producto['cantidad'] ?></span></p>
            </div> 
            <?php }else{ ?>
            <div class="alert alert-light">          
            <span id="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>"><?= $producto['cantidad'] ?></span>
            </div>
            <?php }  ?>  
        </div>
        
        <!-- botones de ok para cada producto -->
        <!-- 21/10/2020 para el escaneo de ean creamos por cada producto un input hidden con class igual al ean de producto (si no tiene ean no pasa nada, se tendrá que marcar a mano) y value la cantidad de producto. Desde javascript buscaremos los elementos del dom con la clase del ean escaneado. Marcaremos el radiobutton success correspondiente como marcado si se escanea el producto-->
      <div class="col-sm-2 col-xs-2" style="padding-top:5%;">
        <div class="btn-group" data-toggle="buttons">      
            <input type="hidden" class="<?= $producto['ean'] ?>" value="<?= $producto['cantidad'] ?>" 
             id="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>_<?= $producto['ean'] ?>" >
            ...
            <!-- 23/10/2020 Añadimos un nuevo radio button warning para gestionar al escanear ean si falta alguna unidad o sobra alguna. Además, si se pulsa, hará reset quitando checked de los radio button de dicho producto y devolviendo al input hidden el value original -->
            <label class="btn btn-warning">
                <input type="radio" name="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>" id="<?= $producto['id_producto'] ?>_<?= $producto['id_atributo'] ?>_<?= $producto['id_order'] ?>" value="2" autocomplete="off" >
                <span class="fa fa-times"></span>
            </label>              
        </div>     
      </div>
...
<!-- Div oculto que se muestra cuando se intenta cerrar un packing con un producto con radio button en warning -->
<div class="row" id="warning_radio_button" style="padding-left:15px; margin-bottom:30px; display:none;"> 
    <p class="h2"><span id="span_warning_radio_button" class="badge badge-warning">Revisa los productos, no puedes cerrar un packing con un producto en alerta</span></p>
</div>
...
<div class="row" style="padding-left:30%; margin-bottom:30px;"> 
    ...
    <button type="submit" name="submit_volver" id="submit_volver" class="btn btn-danger submit_boton_packing"  formnovalidate  style="margin-left:10px;">Cancelar</button>
    </div>
</form>

<!-- 21/10/2020 Añadimos un div sticky con un input para introducir el ean de producto con el scanner, el div deberá ir pegado a la esquina inferior izquierda y cuando sea "lanzado" con un ean, se leerá con javascript para comprobar si el ean introducido se encuantre entre los productos del pedido y en ese caso marcarlo como OK -->


<div class="sticky_ean">
    <div class="row" id="sticky_producto_escaneado" style="display:none;">
        <div id="escaneo_success" style="display:none;">            
            <img id="imagen_success" src=""  width="150" height="200"/>
            <p>EAN CORRECTO</p> 
            <p>Producto marcado OK correctamente</p> 
        </div>
        <div id="escaneo_danger" style="display:none;">
            <p>EAN INCORRECTO</p> 
            <p>El Ean no corresponde a ningún producto del pedido</p>        
        </div>
        <div id="escaneo_warning_faltan" style="display:none;"> 
            <img id="imagen_faltan" src=""  width="150" height="200"/>           
            <p>EAN CORRECTO</p>
            <p>FALTAN UNIDADES</p>                   
        </div>
        <div id="escaneo_warning_sobran" style="display:none;"> 
            <img id="imagen_sobran" width="150" height="200"/>           
            <p>EAN CORRECTO</p>
            <p>SOBRAN UNIDADES</p>            
            <p>Hecho picking de demasiadas unidades o marcado OK a mano.</p>       
        </div>
    </div>
    <div class="row">
        <form id="sticky_form_ean" action="">
            <label for="input_ean">Ean13</label>        
            <input type="text" id="input_ean" maxlength="14" size="14" autofocus>
            <input type="submit" id="submit_sticky_form_ean" value="Ok">        
        </form>
    </div>
</div>
