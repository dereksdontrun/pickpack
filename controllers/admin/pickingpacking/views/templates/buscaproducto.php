<!-- /**
* Utilidad para hacer picking y packing prescindiendo de hojas de pedido
*
* @author    Sergio™ <sergio@lafrikileria.com>
*/ -->
<?php

include('header.php');

?>

<div class="jumbotron jumbotron_ubicaciones" style="padding-top:10px;">
    <h1 class="display-4">Hola <?= $_SESSION["nombre_empleado"] ?>  
        
    <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown"><?= $_SESSION["nombre_empleado"] ?>
    <span class="caret"></span></button>
    <ul class="dropdown-menu">
        <!-- quito variable get ?cerrar_sesion=1 de momento -->
        <li><a class="dropdown-item" href="<?= _MODULE_DIR_.'pickpack/controllers/admin/pickingpacking/pickpackindex.php'; ?>">  Cerrar Sesión</a></li>              
    </ul>

    </h1>
    <p class="lead">Ubicaciones de La Frikilería</p>

    <?php
    if ($mensaje_ok){
    ?>
    <p class="lead"><span style="font-size: 17px;" class="badge badge-pill badge-success">¡Localizaciones actualizadas!</span></p><br>
    <?php
    }
    ?>

    <?php
    if ($incidencia){
    ?>
    <p class="lead"><span style="font-size: 17px;" class="badge badge-pill badge-warning"><?= $mensaje_incidencia ?></span></p><br>
    <?php
    }
    ?>
    
    <div class="container" style="margin-bottom:20px;">  
    <form action="ubicaciones.php" method="post"> 
        <div class="form-group">         
            <label for="text">Introduce el Ean del producto a ubicar:</label> 
        
            <input type="text" name="ean" size="30"  class="form-control" autofocus>
        </div>
        <button type="submit" name="submit_ean" class="btn btn-primary">Buscar</button>
    </form>
    </div>  
</div>